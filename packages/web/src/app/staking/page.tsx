...
export default function StakingPage() {
  ...
  const status = vaultStats ? solvencyStatus(vaultStats.solvencyRatio * 10000) : SolvencyStatus.Healthy;

  return (
    <div style={{ padding: 'var(--space-xl)', maxWidth: 1200, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: 'var(--space-xl)' }}>Staking Pool</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-lg)' }}>
        ...
        <Card title="Pool Health">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-lg)' }}>
            <div>
              <p style={{ color: 'var(--color-text-muted)', fontSize: '0.8125rem' }}>Total Staked</p>
              <p style={{ fontSize: '1.5rem', fontWeight: 700 }}>${vaultStats ? formatUsdc(vaultStats.totalStaked) : '0.00'}</p>
            </div>
            <div>
              <p style={{ color: 'var(--color-text-muted)', fontSize: '0.8125rem' }}>Total Coverage</p>
              <p style={{ fontSize: '1.5rem', fontWeight: 700 }}>${vaultStats ? formatUsdc(vaultStats.totalCoverage) : '0.00'}</p>
            </div>
            <div>
              <p style={{ color: 'var(--color-text-muted)', fontSize: '0.8125rem' }}>Solvency Ratio</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
                <p style={{ fontSize: '1.5rem', fontWeight: 700 }}>{vaultStats?.solvencyRatio?.toFixed(2) ?? '0'}x</p>
        ...
        <Card title="Stake USDC">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
            <div>
              <label style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)', display: 'block', marginBottom: 4 }}>Amount (USDC)</label>
              <input
                type="number"
                value={stakeAmount}
      ...
      <Card title="My Position" style={{ marginTop: 'var(--space-lg)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--space-lg)' }}>
          ...
          <div>
            <p style={{ color: 'var(--color-text-muted)', fontSize: '0.8125rem' }}>Pending Rewards</p>
            <p style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--color-primary)' }}>$0.00</p>
          </div>
          <div>
